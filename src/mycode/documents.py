...
def render_understanding(report: DiagnosticReport, edition: int) -> str:
    ...
    # LLM fix caveat (only when at least one finding has a suggestion)
    if any(f.llm_fix_suggestion for f in report.findings):
        lines.append("---")
        lines.append("")
        lines.append(
            "Some fix suggestions in this report were generated by an AI "
            "model (Gemini) based on your source code and the diagnostic "
            "findings. These suggestions are starting points, not guaranteed "
            "fixes. Always review AI-generated suggestions in the context of "
            "your full codebase before applying them."
        )
        lines.append("")
